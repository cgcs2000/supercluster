...
test('generates unique ids with generateId option', (t) => {
    ...
    t.same(ids, [22, 24, 28, 30, 118, 125, 118]);
    t.end();
});
...
test('getLeaves handles null-property features', (t) => {
    ...
    const leaves = index.getLeaves(165, 1, 2);
...
test('returns cluster expansion zoom', (t) => {
    ...
    t.same(index.getClusterExpansionZoom(228), 1);
    t.same(index.getClusterExpansionZoom(260), 1);
    t.same(index.getClusterExpansionZoom(292), 1);
    t.end();
});
...
test('returns cluster expansion zoom for maxZoom', (t) => {
    ...
    t.same(index.getClusterExpansionZoom(2599), 4);
    t.end();
});
